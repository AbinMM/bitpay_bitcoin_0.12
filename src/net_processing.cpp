    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (nServices & NODE_BITCOIN_CASH) {
            if (GetTime() < 1533096000) {
                // Give operators the freedom to Immediately disconnect peers that use service bit
                // 5 until August 1st, 2018. Using this option with a code that advertises said
                // service bits is probably a bad idea.
                LogPrint(BCLog::NET, "peer=%d offering service bit 5, disconnecting\n", pfrom->GetId());
                pfrom->fDisconnect = true;
                return false;
            }
        }
